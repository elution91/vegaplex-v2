    ...
    def compute_recommendation(self, ticker: str) -> Dict:
        ...
        try:
            ...
            for exp_date, chain in options_chains.items():
                ...
                dte = (datetime.strptime(exp_date, "%Y-%m-%d").date() - today_dt).days
                if dte <= 0:
                    continue
                ...
                # yfinance returns impliedVolatility≈0 and bid/ask=0 outside market hours.
                # Fall back to lastPrice (prior close) for straddle approximation:
                #   IV ≈ time_value_straddle / (spot * sqrt(T)) * sqrt(2π)
                # Threshold 0.05 (5%) catches clearly bogus pre-market values
                # (real equities almost never have IV below 5% annualised)
                if atm_iv < 0.05:
                    import math
                    c_bid = float(calls.loc[call_idx, 'bid'] or 0)
                    c_ask = float(calls.loc[call_idx, 'ask'] or 0)
                    p_bid = float(puts.loc[put_idx, 'bid'] or 0)
                    p_ask = float(puts.loc[put_idx, 'ask'] or 0)
                    c_mid = (c_bid + c_ask) / 2
                    p_mid = (p_bid + p_ask) / 2
                    # bid/ask also zero pre-market — fall back to lastPrice
                    if c_mid == 0:
                        c_mid = float(calls.loc[call_idx, 'lastPrice'] or 0)
                    if p_mid == 0:
                        p_mid = float(puts.loc[put_idx, 'lastPrice'] or 0)
                    # Strip intrinsic value so we're left with pure time value
                    c_strike = float(calls.loc[call_idx, 'strike'])
                    p_strike = float(puts.loc[put_idx, 'strike'])
                    c_intrinsic = max(0.0, current_price - c_strike)
                    p_intrinsic = max(0.0, p_strike - current_price)
                    c_time = max(0.0, c_mid - c_intrinsic)
                    p_time = max(0.0, p_mid - p_intrinsic)
                    straddle_time = c_time + p_time
                    T = dte / 365.0
                    if straddle_time > 0 and current_price > 0 and T > 0:
                        atm_iv = straddle_time / (current_price * math.sqrt(T)) * math.sqrt(2 * math.pi)

                if atm_iv > 0.01:
                    atm_ivs[exp_date] = atm_iv

                if first_chain:
                    # Straddle price — fall back to lastPrice when bid/ask are zero pre-market
                    c_b = float(calls.loc[call_idx, 'bid'] or 0)
                    c_a = float(calls.loc[call_idx, 'ask'] or 0)
                    p_b = float(puts.loc[put_idx, 'bid'] or 0)
                    p_a = float(puts.loc[put_idx, 'ask'] or 0)
                    call_mid = (c_b + c_a) / 2 if (c_b > 0 or c_a > 0) else float(calls.loc[call_idx, 'lastPrice'] or 0)
                    put_mid  = (p_b + p_a) / 2 if (p_b > 0 or p_a > 0) else float(puts.loc[put_idx,  'lastPrice'] or 0)
